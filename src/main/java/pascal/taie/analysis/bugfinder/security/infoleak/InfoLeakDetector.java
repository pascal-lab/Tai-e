...
package pascal.taie.analysis.bugfinder.security.infoleak;
...
public class InfoLeakDetector extends ProgramAnalysis<Set<BugInstance>> implements Detector {

    public static final String ID = "info-leak";

    private static final Logger logger = LogManager.getLogger(InfoLeakDetector.class);

    private final TaintResultHandler taintResultHandler;

    public InfoLeakDetector(AnalysisConfig config) throws IOException {
        super(config);
    ...
    public Set<BugInstance> analyze() {
        ...
        logger.info("Detected {} information leak point(s):", bugInstances.size());
