...
class CISolver {
    ...
    CISolver(PointerAnalysisResult ptaResult) {
    ...
    SideEffect solve() {
        ...
        Queue<Pair<JMethod, Set<Obj>>> workList = initialize();
        propagate(workList);
        return canonicalizeResult();
    }

    private Queue<Pair<JMethod, Set<Obj>>> initialize() {
        // initializes work list, computes stmtMod of StoreField/Array
        Queue<Pair<JMethod, Set<Obj>>> workList = new ArrayDeque<>();
        for (JMethod m : callGraph) {
            ...
            if (!mMod.isEmpty()) {
                workList.add(new Pair<>(m, mMod));
            }
        }
        return workList;
    }

    private void propagate(Queue<Pair<JMethod, Set<Obj>>> workList) {
        while (!workList.isEmpty()) {
            Pair<JMethod, Set<Obj>> entry = workList.poll();
            JMethod m = entry.first();
            Set<Obj> Objs = entry.second();
            Set<Obj> methodDelta = addToMod(methodMods, m, Objs);
            if (!methodDelta.isEmpty()) {
                for (Invoke invoke : callGraph.getCallersOf(m)) {
                    Set<Obj> invokeDelta = addToMod(stmtMods, invoke, methodDelta);
                    if (!invokeDelta.isEmpty()) {
                        if (isAnalyzable(invoke.getContainer()))
                            workList.add(new Pair<>(invoke.getContainer(), invokeDelta));
                    }
                }
            }
        }
