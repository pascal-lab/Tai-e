 ...
 */
class JArray implements JValue {

    private JValue[] arr;

    private final ArrayType type;

    JArray(JValue[] arr, Type baseType, int dims) {
        this.type = World.get().getTypeSystem().getArrayType(baseType, dims);
        this.arr = arr;
    }

    JArray(JArray another)  {
        this(another.arr.clone(), another.type.baseType(), another.type.dimensions());
    }

    static JArray createArray(int count, Type baseType, int dims) {
    ...
    static JArray createMultiArray(ArrayType at, List<Integer> counts, int countIdx) {
