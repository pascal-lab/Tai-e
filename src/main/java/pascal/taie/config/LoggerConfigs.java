...
public final class LoggerConfigs {
    ...
    /**
     * Re-fetch configuration from log4j2.yml and reconfigure the log4j2 logger.
     */
    public static void reconfigure() {
        Configuration config = ((LoggerContext) LogManager.getContext(false))
                .getConfiguration();
        if (config instanceof YamlConfiguration yamlConfig) {
            config = yamlConfig.reconfigure();
            config.getAppenders().values().forEach(LifeCycle::stop);
