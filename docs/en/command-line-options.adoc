...
** Specify the version of Java library used in the analyses. If `--jre-dir` is given, Tai-e uses the library in that directory; otherwise, Tai-e locates the corresponding library in submodule `java-benchmarks`.
** If `-java` is not given, Tai-e uses the current Java runtime.

* JRE directory (--jre-dir): `--jre-dir <dir>`
** Specify a JRE directory to be used for analysis. This option must be used together with `-java`.
...
----
./tai-e-<version>/bin/tai-e -cp foo.jar -cp [underline]#"my program/dir/"# -m baz.Main -java 8 -a [underline]#"pta=cs:2-type;time-limit:60;"#
----
