...
** Enable periodic sampling of CPU and memory usage during execution.
** Results are saved as `tai-e-performance.json` in the output directory, containing system info, execution metadata, and time-series performance data. Useful for analyzing and comparing resource usage across runs.
