...
== Signature Pattern
Sometimes, the user need to specify multiple relevant classes or members in the configuration, e.g., <<taint-analysis#configuring-taint-analysis,taint analysis>>.
To ease such configuration, we design and implement _signature pattern_ mechanism (akin to regular expression, tailored for class or member), which allows the user to specify multiple relevant classes or members with a single signature pattern.
In this section, we introduce the formats of signature patterns and how to use it in analysis development.
...
This allows users to specify multiple related classes or members using a single signature pattern.
...
To simplify specifying these names, we introduce the concept of *name wildcards*, which are the core of signature patterns.

A name wildcard is simply a name that includes one or more `\*` characters, where each `\*` can match zero or more characters within the name.
For example, `java.util.\*` can match the names of all classes in the `java.util` package and its sub-packages, such as `java.util.regex`.
Similarly, `get*` can match all method names that start with `get`, like `getName` or `getKey`.
