...
A name wildcard is simply a name that includes one or more `\*` characters, where each `\*` can match zero or more characters in the name.
For example, `java.util.\*` matches the names of all classes in the `java.util` package and its sub-packages, such as `java.util.regex`.
Similarly, the wildcard `get*` matches all method names starting with `get`, such as `getName` and `getKey`.
