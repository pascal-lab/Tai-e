...
As mentioned in <<develop-new-analysis#develop-new-analysis,Develop A New Analysis>>, to add a new analysis to Tai-e, one needs to register its information in analysis configuration file `src/main/resources/tai-e-analyses.yml`. Each analysis entry consists of five (or less) attributes:
...
As described in <<command-line-options#command-line-options,Command-Line Options>>, there are two approaches to specify the analyses to execute. Next, we will explain how they affect the generated analysis plan.
