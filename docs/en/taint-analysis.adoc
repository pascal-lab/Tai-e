...
NOTE: Some configuration values contain special characters, e.g., spaces, `[` and `]`.
To ensure that they can be properly processed by YAML parser, remember to *quote* such values.
...
We highlight [underline]#`type: TYPE`# by [underline]#underline# to represent that it is *optional* in call source configuration.
...
NOTE: In the rest of this documentation, we also use [underline]#underline# to represent *optional*.
